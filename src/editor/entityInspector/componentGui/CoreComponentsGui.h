...
struct ComponentInspectorGui<CTransform>
{
    static void Display(CTransform& transform, const std::function<void()>& RemoveComponentCallback, bool* isDirty = nullptr)
    {
        if (ComponentHeader<CTransform>(
                "Transform", &transform, RemoveComponentCallback, [&] { transform = CTransform{}; }, isDirty)) {
            if (ImGui::BeginTable("CTransformTable", 2, ImGuiTableFlags_SizingFixedFit)) {
                TableNextField("Position");
                EntityInspectorHelpers::DragFloat2("##PosX", &transform.position.x, "##PosY", &transform.position.y, 0.1f,
                                                   isDirty);
                TableNextField("Scale");
                EntityInspectorHelpers::DragFloat2("##ScaleX", &transform.scale.x, "##ScaleY", &transform.scale.y, 0.1f,
                                                   isDirty);
...
struct ComponentInspectorGui<CMovement>
{
    ...
    {
        if (ComponentHeader<CMovement>(
                "Movement", &movement, RemoveComponentCallback, [&] { movement = CMovement{}; }, isDirty)) {
