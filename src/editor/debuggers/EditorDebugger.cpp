...
void EditorDebugger::RegisterEditor(Editor* editor)
{
    m_editorPtr = editor;
}
void EditorDebugger::UnRegisterEditor()
{
    m_editorPtr = nullptr;
}
...
void EditorDebugger::DrawTab(DebugTabContext& context)
{
    ...
    if (ImGui::BeginTabItem("Editor"))
    {
        ...
        ImGui::SetCursorPosX(tabWidth -
                             2 * (ImGui::GetFrameHeight() + ImGui::GetStyle().ItemInnerSpacing.x + ImGui::CalcTextSize("Snap to Grid").x));
        ...
        ImGui::SetCursorPosX(tabWidth -
                             (ImGui::GetFrameHeight() + ImGui::GetStyle().ItemInnerSpacing.x + ImGui::CalcTextSize("Show Grid").x));
        ...
        {
            color = sf::Color(static_cast<uint8_t>(col[0] * 255), static_cast<uint8_t>(col[1] * 255), static_cast<uint8_t>(col[2] * 255),
