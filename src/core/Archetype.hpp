...
private:
	...
	EntityLocation AddEntityImplementation(Tuple& componentValues, std::index_sequence<I...>)
	{
		...
		((std::get<Components*>(chunk.ptrToComponentArrayTuple)[index] =
			std::move(std::get<I>(componentValues))), ...);
...
public:
	...
	{
		return AddEntityImplementation(*static_cast<std::tuple<Components...>*>(componentTuple), std::index_sequence_for<Components...>{});
	}
