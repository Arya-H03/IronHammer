...
public:
    ...
    bool ValidateEntity(Entity entity, bool log = true) const
    {
        ...
        if (!isGenValid) {
            ...
            if (log) {
                LOG_WARNING(
                    std::format("Tried to valid Entity({},{}) with invalid Generation.", entity.id, entity.generation));
            }
#endif
            return false;
        }
    ...
    void CreateEntity(Entity entity, Components&&... components)
    {
        ...
        EntityStorageLocation  newEntityArchetypeStorage = archetype.AddEntity(entity, components...);

        // Update Entity Storage Location
        m_entityStorageLocations[entity.id] = newEntityArchetypeStorage;
    }

    void CreateEntity(Entity entity, std::vector<PendingComponent>& pendingComponents)
    {
        // Find Archetype
        ComponentSignatureMask signature                 = ComponentRegistry::MakeSignatureMask(pendingComponents);
        Archetype&             archetype                 = m_archetypeRegistry.GetArchetype(signature);
        EntityStorageLocation  newEntityArchetypeStorage = archetype.AddEntity(entity, pendingComponents);
    ...
    Json SerializeEntity(EntityStorageLocation entityLocation)
    {
        ...
        Archetype& archetype = m_archetypeRegistry.GetArchetypeById(entityLocation.archetypeId);
        archetype.ForEachComponent(entityLocation, [&](ComponentId id, void* ptr) {
