...
void EntityInspector::DrawComponentDisplay(World& currentWorld, ComponentId componentId, void* componentPtr) const
{
    ComponentRegistry::GetComponentInfoById(componentId)
        .DisplayComponent(
            componentPtr, [&]() { currentWorld.RemoveFromEntity(m_currentLiveEntity, componentId, componentPtr); }, nullptr);
}

void EntityInspector::DrawInspectorGuiForLiveEntity(EntityManager&         entityManager,
                                                    EntityTemplateManager& entityTemplateManager, World& currentWorld)
{
    ...
    ImGui::Text("Id:%u | Chunk:%u | Index:%u", entityLocation.archetypeId, entityLocation.chunkIndex,
                entityLocation.indexInChunk);

    ImGui::Spacing();
    ImGui::Spacing();

    archetypePtr->ForEachComponent(entityLocation,
                                   [&](ComponentId id, void* ptr) { DrawComponentDisplay(currentWorld, id, ptr); });
    ...
    if (ImGui::BeginTable("EntityTemplateTable", 2, ImGuiTableFlags_SizingFixedFit)) {
        ...
        if (ImGui::Button("Create")) {
            if (name.empty()) {
                name = "Entity_" + std::to_string(m_currentLiveEntity.id) + "_" +
                       std::to_string(m_currentLiveEntity.generation);
            }
...
void EntityInspector::DrawInspectorGuiForEntityTemplate(EntityTemplateManager& entityTemplateManager)
{
    ...
    if (ImGui::Button("Save")) { m_currentEntityTemplateInstance->Save(entityTemplateManager); }
    if (dirtyFlagTemp) { ImGui::PopStyleColor(); }
...
void EntityInspector::DrawInspectorGui(EntityManager& entityManager, EntityTemplateManager& entityTemplateManager,
                                       World& currentWorld, EngineMode engineMode)
{
